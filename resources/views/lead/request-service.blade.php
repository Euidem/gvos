...
<div class="w-full max-w-6xl mx-auto">
    ...
    <div class="text-center mb-8">
        ...
        <p class="text-on-primary-container text-xs uppercase tracking-wider mt-1">Operations Management Platform</p>
    </div>
    ...
    <div class="flex flex-col-reverse lg:flex-row gap-8 lg:items-start">
        ...
        <div class="flex-1 min-w-0">
            <div class="bg-white rounded-xl shadow-xl overflow-hidden">
                ...
                {{-- Progress header --}}
                <div class="bg-secondary px-6 sm:px-8 py-6">
                    <div class="flex items-start justify-between mb-4">
                        <div>
                            <h1 class="text-xl font-bold text-white leading-snug">Find the right remote support</h1>
                            <p class="text-secondary-fixed/70 text-sm mt-0.5">
                                Step <span id="step-current">1</span> of 4 —
                                <span id="step-label-inline">Your details</span>
                            </p>
                        </div>
                        <span class="text-xs text-secondary-fixed/70 border border-secondary-fixed/40 rounded-full px-3 py-1 mt-0.5 hidden sm:inline-flex whitespace-nowrap">
                            No payment required
                        </span>
                    </div>
                ...
                <form method="POST" action="{{ route('lead.request-service.store') }}"
                    ...
                    @if ($errors->any())
                    <div class="bg-status-blocked/10 border border-status-blocked/20 rounded-xl px-5 py-4 mb-6">
                        <div class="flex items-start gap-2.5">
                            <svg class="w-4 h-4 text-status-blocked mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                            <div>
                                <p class="text-sm font-semibold text-status-blocked mb-1">Please correct the following:</p>
                                <ul class="text-sm text-status-blocked list-disc list-inside space-y-0.5">
                    ...
                    <div class="step-panel" id="step-1">
                        <div class="mb-7">
                            <h2 class="text-2xl font-bold text-on-surface">Tell us about yourself</h2>
                            <p class="text-sm text-on-surface-variant mt-1.5">We use this to set up your account and reach out with next steps.</p>
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">

                            <div>
                                <label for="first_name" class="block text-sm font-semibold text-on-surface mb-1.5">
                                ...
                                <input id="first_name" type="text" name="first_name"
                                       ...
                                       class="w-full px-4 py-3 border rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-secondary/20 transition-colors
                                              @error('first_name') border-status-blocked bg-red-50 @else border-border-subtle @enderror">
                                @error('first_name')<p class="mt-1 text-xs text-status-blocked">{{ $message }}</p>@enderror
                            </div>

                            <div>
                                <label for="last_name" class="block text-sm font-semibold text-on-surface mb-1.5">
                                ...
                                <input id="last_name" type="text" name="last_name"
                                       ...
                                       class="w-full px-4 py-3 border rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-secondary/20 transition-colors
                                              @error('last_name') border-status-blocked bg-red-50 @else border-border-subtle @enderror">
                            ...
                            <div class="sm:col-span-2">
                                <label for="email" class="block text-sm font-semibold text-on-surface mb-1.5">
                                    Email Address <span class="text-status-blocked">*</span>
                                </label>
                                <input id="email" type="email" name="email"
                                       value="{{ old('email') }}" maxlength="255" placeholder="jane@example.com" autocomplete="email"
                                       class="w-full px-4 py-3 border rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-secondary/20 transition-colors
                                              @error('email') border-status-blocked bg-red-50 @else border-border-subtle @enderror">
                                <p class="text-xs text-outline mt-1">We'll send confirmation here. No spam, ever.</p>
                                @error('email')<p class="mt-1 text-xs text-status-blocked">{{ $message }}</p>@enderror
                            </div>

                            <div>
                                <label for="phone" class="block text-sm font-semibold text-on-surface mb-1.5">
                                    Phone Number <span class="text-xs font-normal text-outline">(optional)</span>
                                </label>
                                <input id="phone" type="tel" name="phone"
                                       value="{{ old('phone') }}" maxlength="50" placeholder="+234 800 000 0000" autocomplete="tel"
                                       class="w-full px-4 py-3 border border-border-subtle rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-secondary/20 transition-colors">
                            </div>

                            <div>
                                <label for="country" class="block text-sm font-semibold text-on-surface mb-1.5">
                                    Country <span class="text-xs font-normal text-outline">(optional)</span>
                                </label>
                                <select id="country" name="country"
                                        class="w-full px-4 py-3 border border-border-subtle rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-secondary/20 transition-colors bg-white">
                            ...
                            <div>
                                <label for="city" class="block text-sm font-semibold text-on-surface mb-1.5">
                                    City <span class="text-xs font-normal text-outline">(optional)</span>
                                </label>
                            ...
                            <div>
                                <label for="timezone_select" class="block text-sm font-semibold text-on-surface mb-1.5">
                                    Your Timezone <span class="text-xs font-normal text-outline">(optional)</span>
                                </label>
                            ...
                            <div id="custom-timezone-wrap" class="{{ $isCustomTz ? '' : 'hidden' }} sm:col-span-2">
                                <label for="timezone_custom" class="block text-sm font-semibold text-on-surface mb-1.5">
                                ...
                                <input id="timezone_custom" type="text"
                                       ...
                                       class="w-full px-4 py-3 border border-border-subtle rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-secondary/20 transition-colors">
                                <p class="text-xs text-outline mt-1">Use a standard timezone identifier, e.g. Asia/Dubai</p>
                            </div>
                    ...
                    <div class="step-panel hidden" id="step-2">
                        <div class="mb-7">
                            <h2 class="text-2xl font-bold text-on-surface">What kind of support do you need?</h2>
                            <p class="text-sm text-on-surface-variant mt-1.5">Choose your account type and describe the role you'd like filled.</p>
                        </div>

                        {{-- Client type --}}
                        <div class="mb-7">
                            <label class="block text-sm font-semibold text-on-surface mb-3">
                                I am requesting support as… <span class="text-status-blocked">*</span>
                            </label>
                            <div class="grid grid-cols-2 gap-4">
                                <label class="cursor-pointer">
                                    ...
                                    <div class="client-type-card border-2 rounded-xl p-5 transition-all
                                        ...
                                        <p class="font-semibold text-on-surface text-sm">Individual</p>
                                        <p class="text-xs text-on-surface-variant mt-0.5">I need support for myself</p>
                                    </div>
                                </label>
                                ...
                                <label class="cursor-pointer">
                                    ...
                                    <div class="client-type-card border-2 rounded-xl p-5 transition-all
                                        ...
                                        <p class="font-semibold text-on-surface text-sm">Business</p>
                                        <p class="text-xs text-on-surface-variant mt-0.5">I represent a company</p>
                                    </div>
                                </label>
                            </div>
                            @error('client_type')<p class="mt-2 text-xs text-status-blocked">{{ $message }}</p>@enderror
                        </div>

                        {{-- Business fields --}}
                        <div id="business-fields"
                             class="{{ old('client_type') === 'business' ? '' : 'hidden' }} grid grid-cols-1 sm:grid-cols-2 gap-5 mb-7 p-5 bg-surface-container-low rounded-xl border border-border-subtle">
                            <p class="text-xs font-semibold text-on-surface-variant uppercase tracking-wide sm:col-span-2">Company Details</p>
                            <div>
                                <label for="company_name" class="block text-sm font-semibold text-on-surface mb-1.5">Company Name</label>
                                <input id="company_name" type="text" name="company_name"
                                       value="{{ old('company_name') }}" maxlength="255" placeholder="Acme Ltd"
                                       class="w-full px-4 py-3 border border-border-subtle rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-secondary/20 bg-white transition-colors">
                            </div>
                            <div>
                                <label for="company_website" class="block text-sm font-semibold text-on-surface mb-1.5">Company Website</label>
                                <input id="company_website" type="url" name="company_website"
                                       value="{{ old('company_website') }}" maxlength="255" placeholder="https://acme.com"
                                       class="w-full px-4 py-3 border border-border-subtle rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-secondary/20 bg-white transition-colors">
                            </div>
                            <div class="sm:col-span-2">
                                <label for="company_email_domain" class="block text-sm font-semibold text-on-surface mb-1.5">Company Email Domain</label>
                                <input id="company_email_domain" type="text" name="company_email_domain"
                                       ...
                                       class="w-full px-4 py-3 border border-border-subtle rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-secondary/20 bg-white transition-colors">
                                <p class="text-xs text-outline mt-1">Used to match staff email addresses during onboarding.</p>
                            </div>
                        </div>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-on-surface mb-3">
                                What role do you need? <span class="text-xs font-normal text-outline">(optional)</span>
                            </label>
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                @php
                                    $roleData = [
                                        ...
                                        'other'                => ['d' => 'M12 6v6m0 0v6m0-6h6m-6 0H6',                                                                                            'bg' => 'bg-slate-100',   'ic' => 'text-on-surface-variant',   'bsel' => 'border-slate-500 bg-surface-container-low'],
                                    ];
                                @endphp
                                @foreach ($roles as $val => $lbl)
                                    @php $r = $roleData[$val] ?? ['d' => 'M12 6v6m0 0v6m0-6h6m-6 0H6','bg'=>'bg-slate-100','ic'=>'text-on-surface-variant','bsel'=>'border-slate-500 bg-surface-container-low']; $sel = old('role_needed') === $val; @endphp
                                    <label class="cursor-pointer">
                                        ...
                                        <div class="role-card border-2 rounded-xl p-3 text-center transition-all hover:shadow-sm
                                                    {{ $sel ? $r['bsel'] : 'border-border-subtle hover:border-border-subtle' }}"
                                             data-bsel="{{ $r['bsel'] }}">
                                            ...
                                            <p class="text-xs font-medium text-on-surface leading-tight">{{ $lbl }}</p>
                                        </div>
                                    </label>
                                @endforeach
                            </div>
                            ...
                            <div id="other-role-wrap" class="{{ old('role_needed') === 'other' ? '' : 'hidden' }} mt-4">
                                <label for="role_needed_other" class="block text-sm font-semibold text-on-surface mb-1.5">Please describe the role</label>
                    ...
                    <div class="step-panel hidden" id="step-3">
                        <div class="mb-7">
                            <h2 class="text-2xl font-bold text-on-surface">What will your assistant help with?</h2>
                            <p class="text-sm text-on-surface-variant mt-1.5">Help us understand your workload so we can match you well. All fields are optional.</p>
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div>
                                <label for="estimated_hours_per_week" class="block text-sm font-semibold text-on-surface mb-1.5">
                                    Estimated Hours / Week <span class="text-xs font-normal text-outline">(optional)</span>
                                </label>
                                <input id="estimated_hours_per_week" type="number" name="estimated_hours_per_week"
                                       ...
                                       class="w-full px-4 py-3 border border-border-subtle rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-secondary/20 transition-colors
                                              @error('estimated_hours_per_week') border-status-blocked bg-red-50 @enderror">
                                <p class="text-xs text-outline mt-1">Typical engagements are 10–40 hours per week.</p>
                                @error('estimated_hours_per_week')<p class="mt-1 text-xs text-status-blocked">{{ $message }}</p>@enderror
                            </div>

                            <div>
                                <label for="preferred_start_date" class="block text-sm font-semibold text-on-surface mb-1.5">
                                    Preferred Start Date <span class="text-xs font-normal text-outline">(optional)</span>
                                </label>
                                <input id="preferred_start_date" type="date" name="preferred_start_date"
                                       value="{{ old('preferred_start_date') }}"
                                       class="w-full px-4 py-3 border border-border-subtle rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-secondary/20 transition-colors
                                              @error('preferred_start_date') border-status-blocked bg-red-50 @enderror">
                                @error('preferred_start_date')<p class="mt-1 text-xs text-status-blocked">{{ $message }}</p>@enderror
                            </div>

                            <div class="sm:col-span-2">
                                <label for="preferred_work_schedule" class="block text-sm font-semibold text-on-surface mb-1.5">
                                    Preferred Work Schedule <span class="text-xs font-normal text-outline">(optional)</span>
                                </label>
                            ...
                            <div class="sm:col-span-2">
                                <label for="required_skills" class="block text-sm font-semibold text-on-surface mb-1.5">
                                    Required Skills <span class="text-xs font-normal text-outline">(optional)</span>
                                </label>
                                <input id="required_skills" type="text" name="required_skills"
                                       ...
                                       class="w-full px-4 py-3 border border-border-subtle rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-secondary/20 transition-colors">
                                <p class="text-xs text-outline mt-1">Separate skills with commas. We use this to find the best match.</p>
                            </div>

                            <div class="sm:col-span-2">
                                <label for="work_description" class="block text-sm font-semibold text-on-surface mb-1.5">
                                    Work Description <span class="text-xs font-normal text-outline">(optional)</span>
                                </label>
                                <textarea id="work_description" name="work_description" rows="5"
                                          ...
                                          class="w-full px-4 py-3 border border-border-subtle rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-secondary/20 resize-y transition-colors
                                                 @error('work_description') border-status-blocked bg-red-50 @enderror">{{ old('work_description') }}</textarea>
                                <p class="text-xs text-outline mt-1">Maximum 5,000 characters.</p>
                                @error('work_description')<p class="mt-1 text-xs text-status-blocked">{{ $message }}</p>@enderror
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="step-panel hidden" id="step-4">
                        <div class="mb-7">
                            <h2 class="text-2xl font-bold text-on-surface">Almost there — final details</h2>
                            <p class="text-sm text-on-surface-variant mt-1.5">No payment required at this stage. This helps us prepare the right estimate for you.</p>
                        </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-on-surface mb-3">
                                    What's your rough monthly budget? <span class="text-xs font-normal text-outline">(optional)</span>
                                </label>
                                ...
                                <p class="text-xs text-outline mt-2">This is a rough guide — your final estimate will be prepared by our team based on your exact requirements.</p>
                            </div>

                            {{-- Source --}}
                            <div>
                                <label for="source" class="block text-sm font-semibold text-on-surface mb-1.5">
                                    How did you hear about GVOS? <span class="text-xs font-normal text-outline">(optional)</span>
                                </label>
                            ...
                            {{-- Privacy note --}}
                            <div class="bg-surface-container-low border border-border-subtle rounded-xl px-5 py-4">
                    ...
                    <div id="step-nav" class="flex items-center justify-between mt-8 pt-6 border-t border-slate-100">
                        <button type="button" id="btn-back"
                                class="hidden items-center gap-1.5 px-5 py-2.5 text-sm font-medium text-on-surface-variant border border-border-subtle rounded-xl hover:bg-surface-container-low active:bg-slate-100 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="w-full lg:w-80 xl:w-96 flex-shrink-0 space-y-5">
            ...
            <div class="bg-white rounded-xl shadow-sm p-7">
                <h2 class="text-lg font-bold text-on-surface leading-snug mb-3">
                ...
                <div class="space-y-3.5">
                    ...
                    <div class="flex items-start gap-3">
                        <div class="w-5 h-5 bg-emerald-100 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                            <svg class="w-3 h-3 text-status-active" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/>
                            </svg>
                        </div>
                        <p class="text-sm text-on-surface-variant">{{ $bullet }}</p>
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            {{-- What happens next --}}
            <div class="bg-white rounded-xl shadow-sm p-7">
                <h3 class="text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-5">What happens next</h3>
                <div class="space-y-5">
                    @foreach ([
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-on-surface">{{ $s['title'] }}</p>
                            <p class="text-xs text-on-surface-variant mt-0.5">{{ $s['sub'] }}</p>
                        </div>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="bg-primary-container rounded-xl p-5 overflow-hidden relative select-none" style="background:linear-gradient(135deg,#0B0F19 0%,#131b2e 60%,#0d1a36 100%)">
                ...
                <div class="bg-white/10 border border-white/15 rounded-xl p-3 mb-2.5 flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-secondary-fixed/70 truncate">Virtual Assistant · 20h/wk</p>
                    </div>
                ...
                <div class="bg-white/10 border border-white/15 rounded-xl p-3 mb-2.5 flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-secondary-fixed/70 truncate">Prequalified · Supervised</p>
                    </div>
                ...
                <div class="bg-white/10 border border-white/15 rounded-xl p-3 mb-2.5">
                    ...
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2">
                            ...
                            <p class="text-xs text-secondary-fixed/70">Email inbox management</p>
                        </div>
                        <div class="flex items-center gap-2">
...
(function() {
    ...
    function markError(el) {
        el.classList.add('border-status-blocked', 'bg-red-50');
        el.focus();
    }
    ...
    document.addEventListener('input', function(e) {
        if (e.target && (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA' || e.target.tagName === 'SELECT')) {
            e.target.classList.remove('border-status-blocked', 'bg-red-50');
        }
    });
