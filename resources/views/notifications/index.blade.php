...
<x-layouts.gvos title="Notifications">
    <div class="max-w-4xl mx-auto">
        ...
        <x-portal.page-header
            title="Notifications"
            subtitle="Updates from your workspaces.">
            <x-slot:actions>
                @if (auth()->user()->unreadNotifications()->exists())
                    <form method="POST" action="{{ route('notifications.read-all') }}">
                        @csrf
                        <x-portal.btn variant="primary" icon="done_all">Mark All Read</x-portal.btn>
                    </form>
                @endif
                <x-portal.btn variant="secondary" icon="tune" :href="route('settings.notifications')">
                    Settings
                </x-portal.btn>
            </x-slot:actions>
        </x-portal.page-header>
