<x-layouts.gvos :title="$workspace->name . ' — Edit Time Log'">
    ...
    <div class="flex items-center gap-2 text-sm text-on-surface-variant mb-5">
        ...
        <span class="material-symbols-outlined" style="font-size: 14px;">chevron_right</span>
        <a href="{{ route('workspace.time-logs.show', [$workspace, $timeLog]) }}" class="hover:text-secondary transition-colors">
            {{ $timeLog->log_date ? $timeLog->log_date->format('d M Y') : 'Log' }}
        </a>
        <span class="material-symbols-outlined" style="font-size: 14px;">chevron_right</span>
        <span>Edit</span>
    </div>
    ...
    <div class="flex items-center justify-between mb-5">
        ...
        <a href="{{ route('workspace.time-logs.show', [$workspace, $timeLog]) }}"
    ...
    <div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-6">
        <form method="POST" action="{{ route('workspace.time-logs.update', [$workspace, $timeLog]) }}" class="space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <input type="date" name="log_date"
                       value="{{ old('log_date', $timeLog->log_date ? $timeLog->log_date->format('Y-m-d') : '') }}"
            ...
            <div>
                ...
                <input type="text" name="work_summary"
                       value="{{ old('work_summary', $timeLog->work_summary) }}"
                       maxlength="1000"
                       class="w-full px-3 py-2 rounded-lg border border-[#E2E8F0] text-sm focus:outline-none focus:ring-2 focus:ring-[#0058be]"
                       required>
            </div>
                    ...
                    <select name="workspace_task_id"
                        ...
                        @foreach ($tasks as $task)
                            <option value="{{ $task->id }}"
                                {{ old('workspace_task_id', $timeLog->workspace_task_id) == $task->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" name="started_at"
                           value="{{ old('started_at', $timeLog->started_at ? $timeLog->started_at->format('H:i') : '') }}"
                           class="w-full px-3 py-2 rounded-lg border border-[#E2E8F0] text-sm focus:outline-none focus:ring-2 focus:ring-[#0058be]">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-on-surface mb-1">End Time (optional)</label>
                    <input type="time" name="ended_at"
                           value="{{ old('ended_at', $timeLog->ended_at ? $timeLog->ended_at->format('H:i') : '') }}"
                           class="w-full px-3 py-2 rounded-lg border border-[#E2E8F0] text-sm focus:outline-none focus:ring-2 focus:ring-[#0058be]">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="work_details" rows="4" maxlength="5000"
                          class="w-full px-3 py-2 rounded-lg border border-[#E2E8F0] text-sm focus:outline-none focus:ring-2 focus:ring-[#0058be] resize-y">{{ old('work_details', $timeLog->work_details) }}</textarea>
            </div>

            {{-- Manager-only fields --}}
            @if (in_array($role, ['admin', 'workspace_admin', 'manager']))
                <div class="p-4 rounded-lg space-y-4" style="background:rgba(0,88,190,0.03);border:1px solid rgba(0,88,190,0.10);">
                    <p class="text-xs font-bold text-outline uppercase tracking-wide">Manager / Admin Fields</p>
                    <div>
                        <label class="block text-xs font-semibold text-on-surface mb-1">Visibility</label>
                        <select name="visibility"
                                class="w-full px-3 py-2 rounded-lg border border-[#E2E8F0] text-sm focus:outline-none focus:ring-2 focus:ring-[#0058be]">
                            <option value="internal" {{ old('visibility', $timeLog->visibility) === 'internal' ? 'selected' : '' }}>Internal only</option>
                            <option value="client_summary" {{ old('visibility', $timeLog->visibility) === 'client_summary' ? 'selected' : '' }}>Show summary to client</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-on-surface mb-1">Client-Visible Summary</label>
                        <textarea name="client_visible_summary" rows="2" maxlength="2000"
                                  class="w-full px-3 py-2 rounded-lg border border-[#E2E8F0] text-sm focus:outline-none focus:ring-2 focus:ring-[#0058be] resize-y">{{ old('client_visible_summary', $timeLog->client_visible_summary) }}</textarea>
                    </div>
                </div>
            @endif
                ...
                <div class="flex items-center gap-4">
                    <label class="flex items-center gap-2 text-sm cursor-pointer">
                        <input type="radio" name="status" value="draft"
                               {{ old('status', $timeLog->status) === 'draft' ? 'checked' : '' }}>
                        Draft
                    </label>
                    <label class="flex items-center gap-2 text-sm cursor-pointer">
                        <input type="radio" name="status" value="submitted"
                               {{ old('status', $timeLog->status) === 'submitted' ? 'checked' : '' }}>
                        Submit for review
                    </label>
                </div>
            </div>
