<x-layouts.gvos title="Create Task">
    ...
    <div class="max-w-2xl">
        <div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden">
            ...
            <div class="p-8">
                <h2 class="text-lg font-bold text-[#191c1e] mb-6 flex items-center gap-2">
                    <span class="material-symbols-outlined text-[#0058be]" style="font-size: 20px;">add_task</span>
                    New Task
                </h2>
                ...
                <form method="POST" action="{{ route('workspace.tasks.store', $workspace) }}" class="space-y-5">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="5"
                                  placeholder="Provide details about this task, what needs to be delivered, and any relevant context..."
                                  class="w-full px-4 py-3 border border-[#E2E8F0] rounded-lg text-sm text-[#191c1e] focus:outline-none focus:ring-2 focus:ring-[#0058be]/20 focus:border-[#0058be] resize-y @error('description') border-status-blocked @enderror">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-status-blocked">{{ $message }}</p>
                        @enderror
                    </div>

                    {{-- Assign to + Priority (2-col) --}}
                                ...
                                @foreach ($members as $member)
                                    @if ($member->user)
                                        <option value="{{ $member->user->id }}"
                                                {{ old('assigned_to_user_id') == $member->user->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select id="priority" name="priority" required
                                ...
                                @foreach (\App\Models\WorkspaceTask::priorityLabels() as $val => $lbl)
                                    <option value="{{ $val }}" {{ old('priority', 'normal') === $val ? 'selected' : '' }}>
                                        {{ $lbl }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        <label for="internal_notes" class="block text-xs font-semibold text-[#45464d] uppercase tracking-wider mb-1.5">
                            Internal Notes
                            <span class="normal-case font-normal text-outline ml-1">(visible to admins and managers only)</span>
                        </label>
                        <textarea id="internal_notes" name="internal_notes" rows="3"
                                  placeholder="Internal notes, instructions or context not visible to clients and talent..."
                                  class="w-full px-4 py-3 border border-[#E2E8F0] rounded-lg text-sm text-[#191c1e] focus:outline-none focus:ring-2 focus:ring-[#0058be]/20 focus:border-[#0058be] resize-y">{{ old('internal_notes') }}</textarea>
                    </div>
                    @endif
