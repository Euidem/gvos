...
## Phase 22 Warnings / Notes

### Widget Route Names
The `OperationalAlertsWidget` and `QuickActionsWidget` use `route('filament.admin.resources.{slug}.index')` and similar patterns. These routes are auto-generated by Filament from resource class names. If any resource class is renamed or if the Filament panel ID changes from `admin`, these route calls will throw a `RouteNotFoundException`. Current resource slugs used: `invoices`, `workspace-subscriptions`, `workspace-time-logs`, `workspace-weekly-reports`, `workspace-vault-items`, `workspaces`, `users`, `payments`, `email-delivery-logs`, `audit-logs`. Page route: `filament.admin.pages.mail-test`.

**Mitigation:** These are admin-only pages. If a route fails, it will error in the admin panel only. Test after deploy by visiting the dashboard.

### Widget Performance
Dashboard widgets load on every page visit to `/admin`. The `OperationalAlertsWidget` runs ~10 queries; `WorkspaceOperationsWidget` runs ~6 queries with `whereHas` (correlated subqueries). These are COUNT queries and should be fast at current scale. If the platform grows to thousands of workspaces, consider adding a simple caching layer (e.g., `Cache::remember('gvos.ops.alerts', 60, fn() => ...)`). Currently no caching is applied — added only if performance degrades.

### AuditLog SelectFilter for Actions
The `AuditLogResource` action filter loads all distinct `action` values from the `audit_logs` table with a limit of 100. As the audit trail grows, this could include many action values. The limit prevents the dropdown from becoming unwieldy.
