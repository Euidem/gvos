...
| generated_at | timestamp nullable | Phase 17: set when draft is auto-generated by WeeklyReportGeneratorService |
| generated_by_user_id | FK users nullable | Phase 17: user who triggered the generation; nullOnDelete |
| created_at / updated_at | timestamps | |
| deleted_at | timestamp nullable | soft deletes |
| INDEX | (workspace_id, status) | |
...
**Phase 17 generation flow:** Manager triggers generation → `WeeklyReportGeneratorService::generate()` → new `draft` record with `generated_at` + `generated_by_user_id` set → manager edits in edit view → publishes via dedicated `POST /{report}/publish` route.
